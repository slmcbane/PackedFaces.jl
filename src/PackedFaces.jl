...
import Base: getindex, setindex!, size, which, similar, strides,
             unsafe_convert
...
strides(A::PackedFaceArray) = strides(A.data)
unsafe_convert(::Type{Ptr{T}}, A::PackedFaceArray{T}) where T =
    unsafe_convert(Ptr{T}, A.data)
